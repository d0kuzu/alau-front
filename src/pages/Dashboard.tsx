...
import { CreditCard, Settings, MessageSquare, Phone, Calendar, TrendingUp, ArrowUp, ArrowDown, BarChart3, MessageCircle, Clock, ArrowRight, FileText } from "lucide-react";
  ...
  return <div className="min-h-screen flex bg-white">
      ...
      <aside className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col fixed left-0 top-0 bottom-0">
        ...
        <div className="p-6 border-b border-slate-200">
          <h1 className="text-2xl font-bold text-slate-900 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate("/")}>
        ...
        <nav className="flex-1 p-4">
          ...
          <div className="space-y-1">
            ...
            return <button key={item.id} onClick={() => setActiveNav(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-[#51C2FB]/10 text-[#51C2FB] border-l-4 border-[#51C2FB]" : "text-slate-700 hover:bg-slate-100"}`}>
        ...
        <div className="p-4 border-t border-slate-200">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-100 transition-colors">
        ...
        <div className="p-8" style={{
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  {["today", "7days", "30days", "60days", "90days"].map(period => <Button key={period} variant={selectedPeriod === period ? "default" : "outline"} size="sm" onClick={() => setSelectedPeriod(period)} className={selectedPeriod === period ? "text-white hover:opacity-90" : "bg-white text-slate-600 hover:bg-slate-50 border-slate-200"} style={selectedPeriod === period ? {
                    background: "linear-gradient(90deg, rgba(113, 181, 234, 1) 0%, rgba(81, 194, 251, 1) 80%)",
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {stats.map((stat, index) => {
                  ...
                  return <Card key={index} className="p-5 bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <p className="text-sm text-slate-600 mb-2">{stat.title}</p>
                          <p className="text-3xl font-bold text-slate-900 mb-1">{stat.value}</p>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {stat.changeType === "positive" ? <ArrowUp className="w-3 h-3 text-green-600" /> : <ArrowDown className="w-3 h-3 text-red-600" />}
            ...
            <Card className="p-6 bg-white border border-slate-200 shadow-sm">
              ...
              <div className="h-[300px] relative" onMouseMove={e => {
                if (hoveredBar !== null) {
                  setTooltipPosition({
                    x: e.clientX,
                    y: e.clientY
                ...
                <div className="absolute left-0 top-0 bottom-12 flex flex-col justify-between pr-2">
                  {[24, 18, 12, 6, 0].map(val => <span key={val} className="text-xs text-slate-500 text-right w-8">
                      {val}
                    </span>)}
                </div>
                ...
                <div className="ml-12 mr-4 h-[calc(100%-3rem)] relative pb-8">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-between">
                    {[0, 6, 12, 18, 24].map(val => <div key={val} className="w-full border-t border-dashed border-slate-200" />)}
                  </div>
                  ...
                  <div className="absolute inset-0 flex items-end justify-between gap-2 px-2">
                    {chartData.map((item, index) => {
                      ...
                      return <div key={index} className="flex-1 flex flex-col items-center justify-end group relative" style={{
                        height: "100%"
                      }} onMouseEnter={e => {
                        setHoveredBar(index);
                        ...
                        if (hoveredBar !== index) {
                          ...
                          <div className="w-full rounded-t-lg transition-all hover:opacity-80 cursor-pointer relative z-10" style={{
                          height: `${height}%`,
              ...
              {/* Tooltip */}
              {hoveredBar !== null && tooltipValue !== null && <div className="fixed bg-slate-900 text-white px-3 py-1.5 rounded text-sm font-medium pointer-events-none z-50" style={{
